import { z } from "zod";
import { taskURL } from "../client.js";
import { filterTasks, isActive, overdue, dueToday, dueThisWeek, hasDuration, today, yesterday, thisWeek, lastWeek, thisMonth, lastMonth, thisQuarter, } from "../filters.js";
...
export function registerAdvancedTools(server, client) {
    server.tool("summarize_completed", "Summarize tasks completed in a time period (today, yesterday, this_week, last_week, this_month, last_month, this_quarter)", {
        period: z
            .string()
            .optional()
            .describe("Time period: today|yesterday|this_week|last_week|this_month|last_month|this_quarter (default today)"),
    }, async (args) => {
        ...
        if (!rangeFn) {
            return {
                content: [{ type: "text", text: `Invalid period: ${period}` }],
                isError: true,
            };
        }
        ...
        const byProject = Array.from(projectCounts.entries()).map(([projectID, count]) => ({
    ...
    server.tool("find_quick_tasks", "Find quick tasks (under specified minutes, default 5 minutes)", {
        max_minutes: z
            .number()
            .optional()
    ...
    server.tool("get_overdue_tasks", "Get all overdue tasks", {}, async () => {
        const tasks = await client.getTasks();
    ...
    server.tool("get_task_stats", "Get overall task statistics (total, active, completed, by priority, etc.)", {}, async () => {
        const tasks = await client.getTasks();
        ...
        for (const task of tasks) {
            ...
            if (overdueFilter(task))
                overdueCount++;
            if (dueTodayFilter(task))
                dueTodayCount++;
            if (dueThisWeekFilter(task))
                dueThisWeekCount++;
        }
        const output = {
            total: tasks.length,
