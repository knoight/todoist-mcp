...
export function registerBasicTools(server: McpServer, client: TodoistClient) {
  server.tool(
    ...
    async (args) => {
      ...
      if (args.project_id) filters.push(byProject(args.project_id));
      if (args.label) filters.push(byLabel(args.label));
      if (args.min_priority) filters.push(byMinPriority(args.min_priority));
  ...
  server.tool(
    "create_task",
    "Create a new Todoist task with optional description, project, due date, and priority",
    {
      ...
      due_string: z
        ...
        .optional()
        .describe(
          "Due date in natural language (e.g. tomorrow, next Monday, Jan 23)"
        ),
      priority: z
        .number()
        .optional()
  ...
  server.tool(
    "update_task",
    "Update an existing Todoist task (content, description, due date, priority)",
    {
